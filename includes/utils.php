...
class Utils {
	...
	public static function get_accept_elements_data() {
		...
		$emails = [
			...
			'customer_cancelled_order'          => $has_order_elements,
	...
	public static function get_order_shortcodes($order_id){
		$result = [];
		...
		$result['refund_html'] = $refund_html;
		$result['payment_method'] = $object && $object->get_total() > 0 && $object->get_payment_method_title() && 'other' !== $object->get_payment_method_title() ? $object->get_payment_method_title() : '';
		...
		$result +=[
			...
			'order_received_url' => $object->get_checkout_order_received_url(),
			'order_shipping' => $object->get_shipping_to_display(),
			'order_discount' => $object->get_discount_to_display(),
			'payment_url' => $object->get_checkout_payment_url(),
			'customer_note' => $object->get_customer_note(),
			'shipping_method' => $object->get_shipping_method(),
			'order_total' => $object->get_formatted_order_total(),
			'order_subtotal' => $object->get_subtotal_to_display(),
			'order_date' => date_i18n( $date_fm, ( $object->get_date_created()->getOffsetTimestamp() ) ),
			'order_date_time' => date_i18n(  "{$date_fm} {$time_format}", ( $object->get_date_created()->getOffsetTimestamp() ) ),
			'customer_phone_number' => $object->get_billing_phone(),
			'customer_email' => $object->get_billing_email(),
			'last_name' => $object->get_billing_last_name(),
			'first_name' => $object->get_billing_first_name(),
			'customer_name' => $object->get_formatted_billing_full_name(),
			'view_order_url' => $object->get_view_order_url(),
			'order_edit_url' => $object->get_edit_order_url(),
			'order_number_with_url_edit' => '<a href="' . $object->get_edit_order_url() . '" >#' . $object->get_order_number() . '</a>',
		...
		if ( self::$rp_object !== $object ) {
			...
			if ( $object ) {
				if ( is_a( $object, 'WC_Order' ) ) {
					...
					if (!isset(self::$cache['order_shortcode_'.$order_id])){
						self::$cache['order_shortcode_'.$order_id] = self::get_order_shortcodes($order_id);
					}
					$order_shortcode = self::$cache['order_shortcode_'.$order_id];
					$user                       = $object->get_user();
					$shortcodes['{user_login}'] = $user->user_login ?? '';
					$shortcodes['{order_id}']                   = $order_id;
					$shortcodes['{order_number}']               = $order_shortcode['order_number'] ?? $object->get_order_number();
					$shortcodes['{order_number_with_url_edit}'] = $order_shortcode['order_number_with_url_edit'] ?? '<a href="' . $object->get_edit_order_url() . '" >#' . $object->get_order_number() . '</a>';
					$shortcodes['{order_edit_url}']             = $order_shortcode['order_edit_url'] ?? $object->get_edit_order_url();
					$shortcodes['{view_order_url}']             = $order_shortcode['view_order_url'] ??  $object->get_view_order_url();
					$shortcodes['{customer_name}']              = $shortcodes['{user_name}']                  = $order_shortcode['customer_name'] ?? $object->get_formatted_billing_full_name();
					$shortcodes['{first_name}']                 = $order_shortcode['first_name'] ?? $object->get_billing_first_name();
					$shortcodes['{last_name}']                  = $order_shortcode['last_name'] ?? $object->get_billing_last_name();
					$shortcodes['{user_email}']                 = $shortcodes['{customer_email}']             = $order_shortcode['customer_email'] ?? $object->get_billing_email();
					$shortcodes['{customer_phone_number}']      = $order_shortcode['customer_phone_number'] ?? $object->get_billing_phone();
					$shortcodes['{order_date_time}']                 = $order_shortcode['order_date_time'] ??'';
					$shortcodes['{order_date}']                 = $order_shortcode['order_date'] ??'';
					$shortcodes['{order_subtotal}']             = $order_shortcode['order_subtotal']??$object->get_subtotal_to_display();
					$shortcodes['{order_total}']                = $order_shortcode['order_total']??$object->get_formatted_order_total();
					$shortcodes['{payment_method}']             = $order_shortcode['payment_method'] ??'';
					$shortcodes['{shipping_method}']            = $order_shortcode['shipping_method']??$object->get_shipping_method();
					$shortcodes['{customer_note}']              = $order_shortcode['customer_note'] ?? $object->get_customer_note() ;
					$shortcodes['{payment_url}']                = $order_shortcode['payment_url'] ?? $object->get_checkout_payment_url();
					$shortcodes['{order_discount}']             = $order_shortcode['order_discount']??$object->get_discount_to_display();
					$shortcodes['{order_shipping}']             = $order_shortcode['order_shipping'] ?? $object->get_shipping_to_display();
					$shortcodes['{order_received_url}']         = $order_shortcode['order_received_url'] ?? $object->get_checkout_order_received_url();
					$shortcodes['{applied_coupon}']             = $order_shortcode['applied_coupon']??'';
					$shortcodes['{order_fully_refund}']         = $shortcodes['{order_partial_refund}']       = $order_shortcode['refund_html'] ?? '';
					$tax = '';
					if ( wc_tax_enabled() ) {
	...
	public static function typed_shortcodes() {
		...
		$sc = [
			...
			'customer_cancelled_order'          => $shortcode_for_order,
			...
			'customer_failed_order'            => $shortcode_for_order,
	...
	public static function check_use_hook_order_download($email_data){
		$result = false;
		...
		if (!isset($email_data['type'])){
			foreach ($email_data as $item){
				if (self::check_use_hook_order_download($item)){
					$result = true;
					break;
				}
			}
		}elseif ($email_data['type'] === 'html/order_detail' && !empty($email_data['attrs']['use_hook_order_download'])){
			$result = true;
		}
		return $result;
	}
