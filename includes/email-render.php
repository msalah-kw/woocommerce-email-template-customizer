...
class Email_Render {
	...
	protected $template_id,$mail_content_html;
...
//		}
	...
	public function parse_styles( $data ) {
		...
		foreach ( $data as $key => $value ) {
            ...
            if ($key ==='background-image'){
	            $style .= "background-repeat: no-repeat; background-position:center top;background-size:cover;";
            }
    ...
    public function final_mail_content($message){
        if ($this->template_id && $this->mail_content_html && empty($this->use_default_template)){
            $message = $this->mail_content_html;
        }
	    $this->mail_content_html = '';
        remove_filter('woocommerce_mail_content',array( $this, 'final_mail_content' ));
        return $message;
    }

	public function render( $data ) {
		$this->check_rendered = true;
        $disable_woo_inline_style =  apply_filters('viwec_disable_woocommerce_email_inline_style', $data['viwec_disable_wc_inline_style'] ?? true, $data);
		$this->custom_css='';
        if ($disable_woo_inline_style){
	        add_filter( 'woocommerce_mail_content', array( $this, 'final_mail_content' ), PHP_INT_MAX, 1 );
            if (empty($data['viwec-preview'])){
                $this->custom_css = get_post_meta( $this->template_id, 'viwec_custom_css', true );
            }
	        add_filter( 'viwec_after_render_style', [ $this, 'custom_style' ] );
        }else{
	        add_filter( 'woocommerce_email_styles', array( $this, 'custom_style' ), 9999 );
        }
		if (apply_filters( 'viwec_enable_woocommerce_email_order_details', class_exists( 'WooCommerce_Germanized' ), $data, $this  ) ) {
		...
		do_action( 'viwec_before_render_email_content', $data, $this->template_id );
		$bg_style   = '';
		$width      = 600;
		$responsive = 414;
		...
		$this->direction = get_post_meta( $this->template_id, 'viwec_settings_direction', true ) ?: 'ltr';

		if ( $this->preview ) {
			$this->direction = isset( $_POST['direction'] ) ? sanitize_text_field( $_POST['direction'] ) : $this->direction ;// phpcs:ignore WordPress.Security.NonceVerification.Missing
		}
		ob_start();
		?>
        <div id="viwec-responsive-wrap">
                        ...
                        <table border="0" cellpadding="0" cellspacing="0" height="100%" align="center"
                            ...
                            <tr>
                                <td align="center" valign="top" id="body_content">
                                    <div id="viwec-responsive-min-width">
											...
											if ( ! empty( $data['rows'] ) && is_array( $data['rows'] ) ) {
												foreach ( $data['rows'] as $row ) {
													if ( ! empty( $row ) && is_array( $row ) ) {
														...
														$block = json_decode( Init::html_entity_decode( $block ), true );
														$rows  = $block['rows'] ?? '';
...
//		echo '<div class="viwec-ios-preview-hidden" style="display: none;color: #ffffff;size: 2pt;">' . wp_strip_all_tags( $email_body_minify ) . '</div>';// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		$this->email_footer();
        $this->mail_content_html = ob_get_clean();
        echo $this->mail_content_html;
		remove_filter( 'viwec_after_render_style', [ $this, 'custom_style' ] );
		do_action( 'viwec_after_render_email_content', $data, $this->template_id );
	}

	public function render_row( $row ) {
		$class_name = $id_name = '';
		$row_outer_style = isset( $row['props']['style_outer']  ) ?  str_replace( "'", '', $row['props']['style_outer']  ) : '';
		if (isset($row_outer_style['class_id_class'])){
			$class_name = $row_outer_style['class_id_class'];
			unset($row_outer_style['class_id_class']);
		}
		if (isset($row_outer_style['class_id_id'])){
			$id_name = $row_outer_style['class_id_id'];
			unset($row_outer_style['class_id_id']);
		}
		$row_outer_style = $this->parse_styles($row_outer_style);
        if ($this->direction == 'rtl'){
            $row_outer_style = 'direction: rtl;'.$row_outer_style;
	        $left_align                =  'right';
        }else{
	        $left_align                = 'left';
        }
        ...
        if ($class_name){
            $class_name .=' viwec-row';
        }
//        ob_start();
        ?>
        <tr>
           <td valign='top' <?php echo esc_attr( $background_image_property ) ?> width='100%' style='<?php echo esc_attr( $row_outer_style ) ?>'
               id="<?php echo esc_attr($id_name ?:'')?>" class="<?php echo esc_attr($class_name ?:'')?>" >
               <table class='viwec-responsive-padding viwec-inline-block' style='border-collapse: collapse;border-spacing:0;margin: 0; padding:0;width: 100%;text-align: center;'>
                   <tr>
                       <?php
                       if (! empty( $row['cols'] && is_array( $row['cols'] ) ) ){
	                       ...
	                       foreach ( $row['cols'] as $key => $col ) {
		                       $col_style = "display:inline-block;margin:0; padding:0;vertical-align: top;width:{$width};text-align:{$left_align};";
		                       $col_style .="line-height: 1.5;box-sizing: border-box;";
		                       $col_style .= ! empty( $col['props']['style'] ) ? $this->parse_styles( $col['props']['style'] ) : '';
                               ?>
                               <td class="viwec-responsive viwec-column" style="<?php echo esc_attr( $col_style ) ?>">
	                               <?php
	                               if ( ! empty( $col['elements'] && is_array( $col['elements'] ) ) ) {
		                               foreach ( $col['elements'] as $el ) {
			                               ...
			                               $class_name = $id_name = '';
			                               $content_style = isset( $el['style'] ) ?  str_replace( "'", '', $el['style'] ) : '';
			                               if (isset($content_style['class_id_class'])){
				                               $class_name = $content_style['class_id_class'];
				                               unset($content_style['class_id_class']);
			                               }
			                               if (isset($content_style['class_id_id'])){
				                               $id_name = $content_style['class_id_id'];
				                               unset($content_style['class_id_id']);
			                               }
			                               $class_name = explode(' ', $class_name);
			                               $content_style = $this->parse_styles($content_style);
			                               $el_style = "padding:0;margin:0;vertical-align:".($type ==='html_image' ?'middle': 'top').';';
			                               $content_style = $el_style.'font-size: 15px;'.$content_style;
//			                               $content_style .= isset( $el['style'] ) ? $this->parse_styles( str_replace( "'", '', $el['style'] ) ) : '';
                                           ...
                                           <table style='border-collapse: collapse;border-spacing: 0;text-align: center;width: 100%'>
                                               <tbody>
                                               <tr>
                                                   <td style='<?php echo esc_attr( $el_style ); ?>'>
                                                       <table id="<?php echo esc_attr($id_name ?:'')?>" class="<?php echo esc_attr( implode(' ',$class_name) ) ?>" style='border-collapse: separate; border-spacing: 0; width: 100%;border: unset;text-align: <?php echo esc_attr( $left_align) ?>;' >
                                                           <tbody>
                                                           <tr>
                                                               <td dir="<?php echo esc_attr( $this->direction ) ?>" style='<?php echo esc_attr( $content_style ) ?>'>
                                                                   <?php
									                               $this->props = $el;
                    ...
                    <tr>
                        <td valign='top' width='100%' class='viwec-responsive-padding viwec-inline-block' border='0' cellpadding='0' cellspacing='0'
							...
							if ( ! empty( $row['cols'] && is_array( $row['cols'] ) ) ) {
								...
								foreach ( $row['cols'] as $key => $col ) {
									ob_start();
                                    $row_style = "display:inline-block;margin:0; padding:0;border-spacing: 0;border-collapse: collapse;vertical-align: top;width:{$width};text-align:{$left_align}";
									$col_style ="line-height: 1.5;box-sizing: border-box;vertical-align: middle;";
									$col_style .= ! empty( $col['props']['style'] ) ? $this->parse_styles( $col['props']['style'] ) : '';
                                    ?>
                                    <table class="viwec-responsive" style="<?php echo esc_attr( $row_style ) ?>">
                                        <tbody>
                                        <tr>
                                            <td class="viwec-column" style='<?php echo esc_attr( $col_style ) ?>'>
                                                <?php
									if ( ! empty( $col['elements'] && is_array( $col['elements'] ) ) ) {
										foreach ( $col['elements'] as $el ) {
											$type = isset( $el['type'] ) ? str_replace( '/', '_', $el['type'] ) : '';
											...
											$el_style = $content_style = "vertical-align:".($type ==='html_image' ?'middle': 'top').';';
                                            ...
                                            <table style='border-collapse: collapse;border-spacing: 0;text-align: center;width: 100%'>
                                                ...
                                                <tr>
                                                    <td style='<?php echo esc_attr( $el_style ); ?>'>
                                                        <table class="<?php echo esc_attr( $type ) ?>" style='border-collapse: separate; border-spacing: 0; width: 100%;border: unset;text-align: <?php echo esc_attr( $left_align) ?>;' >
                                                            <tbody>
                                                            <tr>
                                                                <td dir="<?php echo esc_attr( $this->direction ) ?>" style='<?php echo esc_attr( $content_style ) ?>'>
																	<?php
...
//                                    continue;
                                    ...
                                    <table align="<?php echo esc_attr( $left_align ) ?>" width="<?php echo esc_attr( $width ) ?>"
                                        ...
                                        <tr>
                                            <td>
                                                <table width='100%' align='left' border='0' cellpadding='0' cellspacing='0' style='margin:0; padding:0;border-collapse: collapse;width: 100%'>
                                                    <tr>
                                                        <td class="viwec-column" valign='middle' width='100%'
                                                            style='line-height: 1.5;box-sizing: border-box;<?php echo esc_attr( $col_style ) ?>'>
															<?php
															if ( ! empty( $col['elements'] && is_array( $col['elements'] ) ) ) {
																foreach ( $col['elements'] as $el ) {
																	$type = isset( $el['type'] ) ? str_replace( '/', '_', $el['type'] ) : '';
                                                                    ...
                                                                    <table align='center' width='100%' border='0' cellpadding='0' cellspacing="0" style='border-collapse: collapse;'>
                                                                        <tr>
                                                                            <td valign="<?php echo esc_attr($type ==='html_image' ?'middle': 'top'); ?>" style='<?php echo esc_attr( $el_style ); ?>'>
                                                                                <table class="<?php echo esc_attr( $type ) ?>" align='center' width='100%' border='0' cellpadding='0' cellspacing="0" style='border-collapse: separate; border-spacing: 0;'>
                                                                                    <tr>
...
//                                    echo '<br>';
//                                    echo strlen((string)$b);
//									echo '<br>';
//                                    echo $a;
//									echo '<table style="background: red; color: #fff; font-size: 20px"><tr><td>hello: '.strlen($a).'</td></tr></table>';
//									echo '<table style="background: black; color: #fff; font-size: 20px"><tr><td>hello: '.strlen($b).'</td></tr></table>';
                                    echo $b;
								}
							} ?>
                        </td>
        ...
        $c2 = ob_get_clean();
        echo $c1;
		echo '<tr style="background: red; color: #fff; font-size: 20px;"><td>hi: '.strlen($c1).'</td></tr>';
		echo '<tr style="background: black; color: #fff; font-size: 20px;"><td>hi: '.strlen($c2).'</td></tr>';
        echo $c2;
	}
	...
	public function custom_style($css) {
        if (!$this->custom_css){
            $this->custom_css = '';
        }
		$this->custom_css .= $css;
	...
	public function render_html_text( $props, $arr_style ) {
        ...
        if (apply_filters('viwec_render_html_text_auto_break_line',strpos($content,'{customer_note}'))){
...
//				remove_filter( 'woocommerce_order_item_get_formatted_meta_data', [ $this, 'remove_autop' ] );
	...
	public function render_html_shipping_address_via_hook( $order, $sent_to_admin, $plain_text, $email, $props, $arr_style = [] ) {
        ...
        if (!empty($props['content']['data-shipping_address_title'])){
			...
			$shipping_title = "<tr><td valign='top' class='viwec-shipping-address-title-text' style='font-size:20px;'>{$shipping_title}</td></tr>";
		}

		$html = "{$shipping_title}<tr><td valign='top' style='color: {$color}; font-weight: {$font_weight};font-size:{$font_size};line-height:{$line_height};'>{$shipping_address}{$shipping_phone}</td></tr>";
	...
	public function render_html_suggest_product( $props, $arr_style = [] ) {
		...
		if ( !empty($products) && $row && $column ) {
			...
			$col_width = ($full_width - ( $v_distance * ( $column - 1 ) ) ) /  $column ;
//            ob_start();
            ?><table style="width: 100%;margin: 0; padding: 0;border:unset;border-spacing: 0;border-collapse: collapse"><?php
            for ($i=0;$i<$row;$i++){
                ?>
                    <tr>
                        <?php
                            for ($j = 0; $j < $column; $j++){
                                ...
                                ?>
                        <td class="viwec-responsive suggest-item <?php echo esc_attr( $index ) ?>" style="vertical-align: top;border: 0;padding: 0;margin: 0;font-size: 0;overflow: hidden;width:<?php echo esc_attr(100*$width/$full_width)?>%;">
                            <table  valign='top' style="border:0;border-spacing:0;border-collapse: collapse;width: 100%;<?php echo esc_attr($h_gap)?>">
                                <tr>
			                        <?php
			                        if ( $this->direction !== 'rtl' ) {
				                        ?>
                                        <td class='viwec-mobile-hidden' style='<?php echo esc_attr( $v_gap ) ?>'></td>
				                        <?php
			                        }
			                        ?>
                                    <td style='vertical-align: top; text-align: center;font-size: <?php echo esc_attr( $font_size ) ?>;font-weight: <?php echo esc_attr( $font_weight ) ?>'>
                                        <a href='<?php echo esc_url( $p_url ) ?>' target="_blank" style='text-decoration: none; <?php echo esc_attr( $style ) ?>'>
                                            <img alt='<?php echo esc_attr( $p_name ) ?>' width='<?php echo esc_attr($width)?>'
                                                 style='padding-bottom: 5px; width: 100%;max-width: 100%;' src='<?php echo esc_url( $p_image ) ?>'>
                                            ...
                                            <div style='<?php echo esc_attr( $price_style ) ?>'>
						                        ...
						                        $pattern = '/<span class="screen-reader-text">.*?<\/span>/i';
						                        $p_price =  preg_replace($pattern, '', $p_price);
                        ...
                        <td valign='top' style='font-size: 0;' border='0' cellpadding='0' cellspacing='0'>
							...
							for ( $j = 0; $j < $column; $j ++ ) {
                                ...
                                <table align="left" valign="top" class="viwec-responsive" width="<?php echo esc_attr( $width ); ?>px" border='0' cellpadding='0' cellspacing='0'
                                    ...
                                    <tr style="width: 100%;">
                                        <td class="suggest-item <?php echo esc_attr( $index ) ?>" style="width: 100%;">
                                            <table  valign='top' border='0' cellpadding='0' cellspacing='0' style="border-collapse: collapse;width: 100%;">
                                                <tr>
													<?php
													if ( $this->direction !== 'rtl' ) {
														?>
			...
			$a2 = ob_get_clean();
			echo $a1;
			echo '<tr style="background: red; color: #fff; font-size: 20px;"><td>hi: '.strlen($a1).'</td></tr>';
			echo '<tr style="background: black; color: #fff; font-size: 20px;"><td>hi: '.strlen($a2).'</td></tr>';
			echo $a2;
		}
	}
		...
		if ( isset( $props['attrs']['direction'] ) && $props['attrs']['direction'] === 'vertical' ) {
			foreach ( $socials as $index => $social ) {
				...
				if ( ! empty( $img ) && ! empty( $link ) ) {
					$html .= "<tr><td valign='top' class='social-item {$index}'><a href='{$link}'><img style='vertical-align: middle' alt='{$social}' src='{$img}' width='{$width}'></a></td></tr>";
				}
			}
		} else {
			$html = '<tr>';
			foreach ( $socials as $index => $social ) {
				...
				$img  = isset( $props['attrs'][ $social ] ) ? esc_url( $props['attrs'][ $social ] ) : '';
				if ( ! empty( $img ) && ! empty( $link ) ) {
					$html .= "<td class='social-item {$index}' valign='top' style='padding: 0 3px;'><a href='{$link}'><img alt='{$social}' src='{$img}' width='{$width}'></a></td>";
				}
			}
			$html .= '</tr>';
		}

		$html = "<table class='viwec-no-full-width-on-mobile' align='{$align}' border='0' cellpadding='0' cellspacing='0' style='width: auto !important;'><tbody>$html</tbody></table>";
...
//			if ( is_plugin_active( 'sitepress-multilingual-cms/sitepress.php' ) ) {
	...
	public function render_html_wc_hook( $props, $arr_style = [] ) {
		...
		$this->custom_css .= $this->custom_css ? ".{$hook} table{border-collapse:separate; border:none !important;} blockquote{margin:5px 20px;} .{$hook} img{padding-right:8px;} .{$hook} ul, .{$hook} li{margin:0;} .{$hook} div{margin-bottom:0 !important;}" : '';
		...
		if ( $this->preview ) {
			...
			if ( ! $content ) {
				ob_start();
				?>
                <div class="html_wc_hook_default" style="margin-bottom: 20px;">
            ...
            if ($hook === 'woocommerce_email_order_details' && !$this->use_hook_order_download){
	            $this->use_hook_order_download = Utils::check_use_hook_order_download($this->template_data);
            }
	...
	public function render_html_recover_content( $props, $arr_style = [] ) {
		...
		$this->custom_css .= "#viwec-transferred-content a{color:{$link_color} !important;}";
		$this->custom_css .= "#viwec-transferred-content table.order-item-data tbody td{border-width: 0 !important;}";
		$this->custom_css .= "#viwec-transferred-content table#addresses tbody td{border-width: 0 !important;}";
...
//			$class_email = new \WC_Email();
			...
			$content = '<div id="viwec-transferred-content" class="viwec-no-full-width-on-mobile">' . ( $content ) . '</div>';// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
//			$content = $class_email->style_inline( $content );
		...
		if ( $this->other_message_content ) {
			...
			$content = preg_replace( '/border=[\'"]\d+[\'"]/', 'border="0"', $content );
			echo '<div id="viwec-transferred-content" class="viwec-no-full-width-on-mobile">' . $content . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
		...
		if ( ! empty( $subscriptions && is_array( $subscriptions ) ) ) {
			...
			foreach ( $subscriptions as $subscription ) {
				...
				$cells = [
					...
					esc_html( ( 0 < $subscription->get_time( 'end' ) ) ? date_i18n( wc_date_format(), $subscription->get_time( 'end', 'site' ) ) : _x( 'When cancelled', 'Used as end date for an indefinite subscription', 'viwec-email-template-customizer' ) ),
	...
	public function get_suggest_products( $suggest_type, $limit, $props, $first_variation = false ) {
		...
		$suggest_products = $return_products = $categories_id = $bought_ids = $exclude_products= [];
		$orderby          = 'rand';
        if (!empty($props['attrs']['data-suggest_product-exclude-product'])){
            $exclude_products = explode(',',$props['attrs']['data-suggest_product-exclude-product']);
        }
...
//				if ( is_plugin_active( 'sitepress-multilingual-cms/sitepress.php' ) ) {
			...
			case 'newest':
				...
				break;
			case 'products':
				$suggest_products = explode(',',isset($props['attrs']['data-suggest_product-include-product'])? $props['attrs']['data-suggest_product-include-product'] :'');
				break;
		}
		...
		if (!empty($exclude_products)){
			$suggest_products = array_diff($suggest_products, $exclude_products);
		}
        if (empty($suggest_products) && $suggest_type === 'products'){
            return [];
        }
        ...
        if (empty($suggest_products) && !empty($exclude_products)){
