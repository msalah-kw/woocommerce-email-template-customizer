...
//	    Send test result
	...
	public function builder_page( $post ) {
        ...
        <div id="viwec-right-sidebar">
			<?php
			$boxes = [
				'type'       => esc_html__( 'Email type', 'viwec-email-template-customizer' ),
				'rule'       => esc_html__( 'Rules', 'viwec-email-template-customizer' ),
				'attachment' => esc_html__( 'Attachment files', 'viwec-email-template-customizer' ),
				'testing'    => esc_html__( 'Testing', 'viwec-email-template-customizer' ),
				'admin_note' => esc_html__( "Admin's note for this template", 'viwec-email-template-customizer' ),
				'exim_data'  => esc_html__( "Data", 'viwec-email-template-customizer' ),
			];
	...
	public function viwec_right_sidebar_exim_data() {
        ...
        <div>
            ...
            <div class="vi-ui buttons viwec-btn-group">
                <button type="button" class="vi-ui button tiny attached viwec-import-data"><?php esc_html_e( 'Import','viwec-email-template-customizer' ); ?></button>
                <button type="button" class="vi-ui button tiny attached viwec-export-data"><?php esc_html_e( 'Export' ,'viwec-email-template-customizer'); ?></button>
                <button type="button" class="vi-ui button tiny attached viwec-copy-data"><?php esc_html_e( 'Copy' ,'viwec-email-template-customizer'); ?></button>
            </div>
        </div>
		<?php
	}
	...
	public function viwec_right_sidebar_attachment( $post ) {
        ...
        <div>
            <span class="vi-ui button tiny viwec-add-attachment-file">
                <?php esc_html_e( 'Add file', 'viwec-email-template-customizer' ); ?>
            </span>
        </div>
		<?php
	}
	...
	public function preview_template() {
		...
		$this->custom_css='';
		if (!empty($data['style_container']['responsive'])){
			$this->custom_css ='.viwec-mobile-preview {#body_content {#viwec-responsive-min-width > table {max-width:'.$data['style_container']['responsive'].'px !important}}}';
		}
	...
	public function preview_custom_css($css) {
		if (!$this->custom_css){
			$this->custom_css = '';
		}
		$this->custom_css .= isset( $_POST['custom_css'] ) ? sanitize_text_field( $_POST['custom_css'] ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Missing
		$this->custom_css .= $css;
	...
	public function send_test_email() {
		...
		if (!empty($_POST['email_title'])){
			$subject = wc_clean($_POST['email_title']);
		}else{
			$subject = esc_html__( 'WooCommerce Email Customizer test email template', 'viwec-email-template-customizer' );
		}
		$subject    = apply_filters( 'viwec_sent_test_subject', $subject, $post_ID );
        $subject = Email_Trigger::replace_subject_shortcode($subject,$email_render->order );
		$this->custom_css='';
		add_filter( 'viwec_after_render_style', [ $this, 'preview_custom_css' ] );
		ob_start();
		$email_render->render( $data );
